...
def login():
    ...
    try:
        ...
        success, message, user_data, status_code = AuthService.login_user(
            db, email, password, request.remote_addr
        )
        if success:
            return jsonify(user_data), status_code
        else:
...
def create_event():
    ...
    try:
        success, message, event_data, status_code = EventService.create_event(
            ...
            data.get("location", ""),
            data.get("notes", ""),
            data.get("price_total", 0.0),
        )
        if success:
            return (
                jsonify({"success": True, "message": message, "event": event_data}),
                status_code,
            )
        else:
            return jsonify({"success": False, "message": message}), status_code
...
def cancel_event():
    ...
    try:
        success, message, status_code = EventService.cancel_event(
            db, request.user_id, data.get("event_id")
        )
...
def assign_planner():
    ...
    db = SessionLocal()
    try:
        success, message, event_data, status_code = (
