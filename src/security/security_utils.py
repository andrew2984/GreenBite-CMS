...
def require_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not is_valid:
            return (
                jsonify(
                    {"success": False, "message": payload.get("error", "Invalid token")}
                ),
                401,
            )
...
class SecurityLogger:
    ...
    def log_failed_login(email: str, ip_address: str):
        ...
        print(
            f"[SECURITY] {timestamp} - Failed login attempt for {email} from {ip_address}"
        )
    ...
    def log_successful_login(email: str, ip_address: str):
        ...
        timestamp = datetime.utcnow().isoformat()
        print(
            f"[SECURITY] {timestamp} - Successful login for {email} from {ip_address}"
        )
