...
class EventService:
    ...
    def create_event(
        ...
        try:
            ...
            try:
                ...
                return (
                    False,
                    "Invalid date format. Use ISO format (YYYY-MM-DD HH:mm)",
                    None,
                    400,
                )
    ...
    @staticmethod
    def cancel_event(db, user_id: int, event_id: int) -> Tuple[bool, str, int]:
        ...
        try:
            ...
            event = (
                db.query(Event).filter_by(id=validated_id, client_id=user_id).first()
            )
            if not event:
