...
from ..security.security_utils import InputValidator, JWTManager, SecurityLogger


# service layer class to handle authentication-related business logic
class AuthService:

    PERMISSION_LEVELS = {"client": 0, "planner": 1, "admin": 2}

    ROLE_NAMES = {0: "Client", 1: "Planner", 2: "Admin"}

    @staticmethod
    def register_user(
        db, username: str, email: str, password: str, role: str = "client"
        ...
        try:
            ...
            if existing_user:
                SecurityLogger.log_suspicious_activity(
                    "Duplicate registration attempt", f"Email: {email}"
                )
            ...
            token = JWTManager.generate_token(
                new_user.id, new_user.user_email, role, new_user.permission_lvl
            )
    ...
    @staticmethod
    def login_user(
        db, email: str, password: str, ip_address: str = None
    ) -> Tuple[bool, str, Optional[Dict], int]:
        ...
        try:
            ...
            # Generate JWT token
            token = JWTManager.generate_token(
                user.id, user.user_email, role, user.permission_lvl
            )
            ...
            user_data = {
                ...
                "user": {
                    ...
                    "role": role,
                    "roleDisplay": AuthService.ROLE_NAMES.get(
                        user.permission_lvl, "Unknown"
                    ),
                },
            }
