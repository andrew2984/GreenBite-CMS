...
class AdminService:
    ...
    @staticmethod
    def get_all_events(db, user_id: int) -> Tuple[bool, str, Optional[Dict], int]:
        try:
            ...
            events = sorted(
                all_events, key=lambda e: e.event_date if e.event_date else datetime.max
            )
                ...
                events_data.append(
                    {
                        ...
                        "assigned_planners": (
                            ",".join(assigned_planners) if assigned_planners else ""
                        ),
                        "created_at": (
            ...
            result = {"events": events_data, "planners": planners_data}
    ...
    def assign_planner_to_event(
        db, admin_id: int, event_id: int, planner_id: int
        ...
        try:
            ...
            # Validate IDs
            is_valid, error_msg, validated_event_id = InputValidator.validate_id(
                event_id
            )
            if not is_valid:
                return False, f"Event ID: {error_msg}", None, 400

            is_valid, error_msg, validated_planner_id = InputValidator.validate_id(
                planner_id
            )
            if not is_valid:
            ...
            if admin.assign_planner_to_event(
                db, validated_event_id, validated_planner_id
            ):
                ...
                return (
                    True,
                    "Planner assigned successfully and event status changed to pre-approval",
                    event_data,
                    200,
                )
            else:
                return (
                    False,
                    "Failed to assign planner or planner already assigned",
                    None,
                    400,
                )
    ...
    def assign_multiple_planners(
        db, admin_id: int, event_id: int, planner_ids: List[int]
    ) -> Tuple[bool, str, Optional[Dict], int]:
        ...
        try:
            ...
            for planner_id in planner_ids:
                ...
                planner = (
                    db.query(CMSEventPLanner).filter_by(id=validated_planner_id).first()
                )
                if planner:
            ...
            return (
                True,
                f"{assigned_count} planner(s) assigned successfully",
                event_data,
                200,
            )
